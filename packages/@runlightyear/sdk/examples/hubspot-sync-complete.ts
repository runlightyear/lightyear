...
const crmCollection = defineCollection("crm")
  ...
  .addModel("account", accountSchema, builder => 
    builder
      .withTitle("Company")
      .withMatchPattern({ key: "domain", type: "unique" })
  )
  .addModel("contact", contactSchema, builder => 
    builder
      .withTitle("Contact")
      .withMatchPattern({ key: "email", type: "unique" })
  )
  .addModel("deal", dealSchema, builder => 
    builder.withTitle("Deal")
  )
  .deploy();
...
const hubspotSyncConnector = defineSyncConnector(restConnector, crmCollection)
  ...
  .addModelConnector("account")
    .withList("/crm/v3/objects/companies", {
      ...
      transform: (response) => response.results.map(company => ({
        id: company.id,
        name: company.properties.name || "",
        domain: company.properties.domain || "",
        industry: company.properties.industry || "",
        numberOfEmployees: parseInt(company.properties.numberofemployees) || 0,
  ...
  .addModelConnector("contact")
    .withList("/crm/v3/objects/contacts", {
      responseSchema: hubspotListResponseSchema,
      transform: (response) => response.results.map(contact => ({
        id: contact.id,
    ...
    .withUpdate((id) => `/crm/v3/objects/contacts/${id}`, {
      transform: (contact, previous) => {
        ...
        if (contact.firstName !== previous?.firstName) properties.firstname = contact.firstName;
        if (contact.lastName !== previous?.lastName) properties.lastname = contact.lastName;
        if (contact.phone !== previous?.phone) properties.phone = contact.phone;
        if (contact.jobTitle !== previous?.jobTitle) properties.jobtitle = contact.jobTitle;
        if (contact.lifecycleStage !== previous?.lifecycleStage) properties.lifecyclestage = contact.lifecycleStage;
        return { properties };
      }
    })
  ...
  .addModelConnector("deal")
    .withList("/crm/v3/objects/deals", {
      responseSchema: hubspotListResponseSchema,
      transform: (response) => response.results.map(deal => ({
        id: deal.id,
...
async function exampleUsage() {
  ...
  console.log(`Total deals: ${allDeals.length}, Total value: $${totalDealValue.toLocaleString()}`);
