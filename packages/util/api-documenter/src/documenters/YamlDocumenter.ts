...
export class YamlDocumenter {
  ...
  private _writeTocFile(
    outputFolder: string,
    apiItems: ReadonlyArray<ApiItem>
  ): void {
  ...
  private _generateYamlItem(apiItem: ApiDocumentedItem): IYamlItem | undefined {
    ...
    if (apiItem.tsdocComment) {
      ...
      if (tsdocComment) {
        ...
        for (const exampleBlock of exampleBlocks) {
          const example: string = this._renderMarkdown(
    ...
    switch (apiItem.kind) {
      ...
      case ApiItemKind.EnumMember:
        ...
        if (
          ...
          enumMember.initializerExcerpt.text.length > 0
        ) {
    ...
    if (
      ...
      const associatedPackage: ApiPackage | undefined =
        apiItem.getAssociatedPackage();
  ...
  private _renderInheritance(
    ...
    for (const heritageType of heritageTypes) {
      ...
      const apiItem: ApiItem | undefined =
        this._apiItemsByCanonicalReference.get(type);
      if (apiItem) {
        if (apiItem instanceof ApiClass) {
          if (apiItem.extendsType) {
  ...
  private _renderType(
    ...
    const existingUid: string | undefined =
      yamlReferences.typeNameToUid.get(typeName);
    ...
    if (
      ...
      const apiItem: ApiItem | undefined =
        this._apiItemsByCanonicalReference.get(excerptRef);
